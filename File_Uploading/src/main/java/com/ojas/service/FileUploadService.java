...
//	}
	...
	public String uploadToDB(String name, String message, MultipartFile file, String uploader)
		...
		/*
		 * if user not given uploader name it will take by default name as "guest" and uploaded to root path
		 * 
		 * if user given uploadername it will create a new folder with uploadername and upload to that folder
		 * 
		 */
...
if(!uploader.equals("guest")) {
	...
	if(f2.exists()) {
		f = new File(f2, file.getOriginalFilename());//if already that name folder exist it will not create a new folder just upload to that folder
	}else {
...
/*
 * if mime type ends with mp4 it will allow declared size 
 * normal files also allow the declared size
 */
		...
		/*
		 * to avoid the dublicate files
		 * if the file already exist with same name then we are adding the time with  milliseconds to that file
		 */
		
		String fname = f.getName();
		if(f.exists()) {
			java.util.Date d=new java.util.Date();            
			String s=new SimpleDateFormat("dd-MM-yyyy:HH:mm:ss_").format(d)
                    .replaceAll("-", "_")
                    .replaceAll(":", "_");
...
//			Date date=new Date(ms);
		...
		FileUploadEntity entity = new FileUploadEntity(fname, message, f.getAbsolutePath(),
				kilobytes,	mimeType,uploader, date);
